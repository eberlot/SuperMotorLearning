...
%baseDir = '/Volumes/MotorControl/data/SuperMotorLearning/target/NewTarget';
baseDir = '/Volumes/MotorControl/robotcode/projects/SuperMotorLearning/target/';
seqDir = '/Volumes/MotorControl/data/SuperMotorLearning/stimuli_struct';
...
switch what
    ...
    for e = 1:runs
        ...
        OtherHandRand_G2.lastTrial(length(Trials),:) = 1;
    ...
    test_sess = varargin{1};    % which test - session - different stimuli
        ...
        OtherHandTest_G1.lastTrial([24:24:end],:) = 1;      % every 24th trial - last trial in a block
        OtherHandTest_G2.lastTrial(1:length(Trials),1) = 0;
        OtherHandTest_G2.lastTrial([24:24:end],:) = 1;
    ...
    test_sess = varargin{1};    % which test - session - different stimuli
    subjName1 = varargin{2};
    subjName2 = varargin{3};
    runs = 6;
    trial_run = 24;
    ...
    Trials = 1:(runs*trial_run);  % 4 blocks of 24 - 96
        ...
        dir1=fullfile(baseDir,sprintf('RetentionTest/%s',subjName1));
        dir2=fullfile(baseDir,sprintf('RetentionTest/%s',subjName2));
        dircheck(dir1);
        dircheck(dir2);
        for i = 1:runs
            dsave(fullfile(dir1,['sml_OHT_G1_B', num2str((test_sess-1)*runs+i), '.tgt']),orderfields(getrow(OtherHandTest_G1,B1:B2),OrderFields));
            dsave(fullfile(dir2,['sml_OHT_G2_B', num2str((test_sess-1)*runs+i), '.tgt']),orderfields(getrow(OtherHandTest_G2,B1:B2),OrderFields));
            B1 = B1+trial_run;
            B2 = B2+trial_run; 
        end

    case 'SeqTest_Retention'
        % Testing performance on trained seq, generalisation
        % - Trained sequences
        % - Random sequences (no chunk in common)
        % - Same chunks in sequences, order randomised
...
>>>>>>> Stashed changes
        ...
        SeqTest_G1.lastTrial(1:length(Trials),1) = 0;
        SeqTest_G1.lastTrial([24:24:end],:) = 1;      % every 24th trial - last trial in a block
        SeqTest_G2.lastTrial(1:length(Trials),1) = 0;
        SeqTest_G2.lastTrial([24:24:end],:) = 1;
        ...
        dircheck(dir2);
        for i = 1:runs
            dsave(fullfile(dir1,['sml_STB_G1_B', num2str((test_sess-1)*runs+i), '.tgt']),orderfields(getrow(SeqTest_G1,B1:B2),OrderFields));
            dsave(fullfile(dir2,['sml_STB_G2_B', num2str((test_sess-1)*runs+i), '.tgt']),orderfields(getrow(SeqTest_G2,B1:B2),OrderFields));
            B1 = B1+trial_run;
            B2 = B2+trial_run;
