...
switch what
    ...
    case 'ALPHA:cross_sess_calculate'
        ...
        for s=sn
          ...
          %  A.tessel1       = t.region(A.l1)';
          %  A.tessel2       = t.region(A.l2)';
          %  A.sess1         = t.sessN(A.l1)';
          %  A.sess2         = t.sessN(A.l2)';
    ...
    case 'CONNECT:group_louvain' % depreciated
        ...
        S = reshape(S, N, T);
    case 'CONNECT:group'
        nTessel     = 162; % 162 or 362
        seqType     = 'all'; % all, trained, untrained
        crossType   = 'splithalf'; % alldata or splithalf
...
%         end
    ...
    case 'run_all'
        ...
        for s = 1
            for d=distType
                for st=1:2
                  %  sml_connect('ALPHA:cross_sess_calculate','seqType',sT{st},'nTessel',362,'distType',d{1},'hemi',1,'crossType',crossT{s});
                  %  sml_connect('ALPHA:cross_sess_calculate','seqType',sT{st},'nTessel',362,'distType',d{1},'hemi',1:2,'crossType',crossT{s});
                    for n=nClust
                        sml_connect('CONNECT:group','seqType',sT{st},'nTessel',642,'distType',d{1},'hemiType','CortexBoth','maxClust',n,'crossType',crossT{s});
                        sml_connect('CONNECT:group','seqType',sT{st},'nTessel',642,'distType',d{1},'hemiType','CortexLeft','maxClust',n,'crossType',crossT{s});
                        sml_connect('CONNECT:cluster_dendrogram','seqType',sT{st},'nTessel',362,'distType',d{1},'hemiType','CortexBoth','nCluster',n,'crossType',crossT{s});
                        sml_connect('CONNECT:cluster_dendrogram','seqType',sT{st},'nTessel',362,'distType',d{1},'hemiType','CortexLeft','nCluster',n,'crossType',crossT{s});
                    end
                    fprintf('***** Done all for %s - %s *****\n\n\n',d{1},sT{st});
                end
