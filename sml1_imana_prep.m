   ...
   switch(what)
    ...
    case 'PREP_dicom_folders'
           ...
           for ss=sessN
               % Loop through subjects
               for s = sn;
                   seriesNum = fscanNum{ss}{s};
                   % just task ones
                   seriesNum = seriesNum([1:3,5:7,9:10]);
                   dircheck(fullfile(dicomDir,[subj_name{s},sprintf('_%d',ss)]));
                   cd(fullfile(dicomDir,[subj_name{s},sprintf('_%d',ss)]));
                   
                   % For each series number of this subject (in 'Subject Things')
                   for i=1:length(seriesNum)
                       r     = seriesNum(i);
                       % Get DICOM FILE NAMES
                       if (s<5 || s==5 && ss<3 || s==7 && ss<2)
                           DIR   = dir(sprintf('%s.%4.4d.*.IMA',DicomName{ss}{s},r));   % Get DICOM FILE NAMES
                           ...
                           for ii=1:size(Names,1)
                               copyfile(Names(ii,:),fullfile(dicomDir,[subj_name{s},sprintf('_%d',ss)],sprintf('%d',r),Names(ii,:)));
                               fprintf('%d.',ii);
                           end
                           display(sprintf('\nSeries %d done \n',seriesNum(i)))
                       end
           ...
           cd(cwd);
    case 'run_dicom_job'
         sml1_imana_prep('PREP_dicom_folders','sessN',3);
         sml1_imana_prep('PREP_dicom_folders','sessN',1,'sn',7);
         sml1_imana_prep('PREP_dicom_folders','sessN',2,'sn',7);
    ...
    case 'GLM_contrast_sess'
        ...
        for ss=sessN
            ...
            for s = sn
                ...
                if exist(fullfile(glmSessDir{ss},subj_name{s},'con_AllSeq.nii'))
                    fprintf('All sequence contrast already exists for %s in session-%d -- skipping!\n\n',subj_name{s},ss);
                else
                    ...
                    T   = load('SPM_info.mat');
                    if ~strcmp(SPM.swd,fullfile(glmSessDir{ss},subj_name{s}));
                        SPM=sml1_imana_prep('HOUSEKEEPING_renameSPM','sn',s,'SPM',SPM,'glmDir',glmSessDir{ss});
                    end
                    ...
                    % (4) t contrast of all seq vs. rest
                    con                = zeros(1,size(SPM.xX.X,2));
                    con(:,T.seqNumb>0 & T.seqNumb<13)  = 1;
                    con                = con/sum(con);
    ...
    case 'GLM_make_RepSup'
        ...
        for ss = sessN
            ...
            for s = sn
                ...
                J.volt 			   = 1;
                J.global 		   = 'None';
                J.mask 	           = {fullfile(imagingDir,subj_name{s}, 'rmask_noskull.nii,1')};
                J.mthresh 		   = 0.05;
                J.cvi_mask 		   = {fullfile(imagingDir,subj_name{s},'rmask_gray.nii')};
                J.cvi 			   = cvi_type;
                % Save the GLM file for this subject.
                spm_rwls_run_fmri_spec(J);
                % Save the aux. information file (SPM_info.mat).
...
%                 end 
    ...
    case 'GLM_make_trial'
        ...
        for ss=sessN
            for s = sn
                D = dload(fullfile(behavDir,['sml1_',subj_name{s},'.dat']));
                % Do some subject structure fields.
                dircheck(fullfile(glmTrialDir{ss}, subj_name{s}));
                J.dir 			 = {fullfile(glmTrialDir{ss}, subj_name{s})};
                J.timing.units   = 'secs';                                      % timing unit that all timing in model will be
                J.timing.RT 	 = 1.0;                                         % TR (in seconds, as per 'J.timing.units')
                J.timing.fmri_t  = 16;
                J.timing.fmri_t0 = 1;
    ...
    case 'GLM_estimate_trial'
        ...
        for ss=sessN
            for s = sn
                % Load files
                load(fullfile(glmTrialDir{ss},subj_name{s},'SPM.mat'));
                SPM.swd = fullfile(glmTrialDir{ss},subj_name{s});
                % Run the GLM.
                spm_rwls_spm(SPM);
            end; % sn
        end; %sessN
    ...
    case 'GLM_sess_error_all'
        ...
        for ss=sessN
            for s=sn
                clc;
                tElapsed = tic;
                sml1_imana_prep('GLM_make_sess_error2','sn',s,'glm',glm,'sessN',ss,'numError',6);
                sml1_imana_prep('GLM_estimate_sess_error','sn',s,'sessN',ss,'numError',6);
                sml1_imana_prep('GLM_contrast_sess_error','sn',s,'sessN',ss,'numError',6);
                fprintf('********* Done all %s sess-%d.*********\n\n\n',subj_name{s},ss);
                toc(tElapsed); clear SPM;
            end
        end
        
    case '3g_GLM_RepSup_error' % ------------- GLM for first / second exe, no errors -----------------------
        % repsup analysis, excluding errors
    case 'GLM_all_RepSup_error'
        glm = 3;
        sn = [5:9,11:31];
        sessN = 4;
        vararginoptions(varargin,{'sn','glm','sessN'});
        ...
        for ss=sessN
            for s=sn
                clc;
                tElapsed = tic;
                sml1_imana_prep('GLM_make_RepSup_error','sn',s,'glm',glm,'sessN',ss);
                sml1_imana_prep('GLM_estimate_RepSup_error','sn',s,'sessN',ss);
                sml1_imana_prep('GLM_contrast_RepSup_error','sn',s,'sessN',ss);
                fprintf('********* Done all %s sess-%d.*********\n\n\n',subj_name{s},ss);
                toc(tElapsed); clear SPM; 
            end
        end
    case 'GLM_make_RepSup_error'
        % exclude errors in designing RepSup glm
        % if error on first execution, exclude also the second
        % functional runs - separate regressors for first / second execution
        % makes the GLM file for each subject, and a corresponding
        % SPM_info.mat file. The latter file contains nice summary
        % information of the model regressors, condition names, etc.
        ...
        for ss=sessN % session
            ...
            for s = sn
                ...
                % Do some subject structure fields.
                dircheck(fullfile(glmFoSExErrorDir{ss},subj_name{s})); % add glm type
                J.dir 			 = {fullfile(glmFoSExErrorDir{ss},subj_name{s})};
                J.timing.units   = 'secs';                                      % timing unit that all timing in model will be
                J.timing.RT 	 = 1.0;                                         % TR (in seconds, as per 'J.timing.units')
                J.timing.fmri_t  = 16;
                J.timing.fmri_t0 = 1;
                ...
                for r = 1:numruns_task_sess
                    ...
                    for i = 1:(numTRs(run_task(r))-numDummys)                   % get nifti filenames, correcting for dummy scancs
                        ...
                        N{i} = [fullfile(imagingDir,subj_name{s}, ...
                            [prefix subj_name{s},'_run',runs{ss}{run_task(1,r)},'.nii,',num2str(i)])];
                        
                    end;
                    J.sess(r).scans = N;  % number of scans in run
                    % here adjust error variable
                    % if error on exe 1, don't take exe2
                    errorTrial = zeros(size(R.isError));
                    errorTrial(R.numError>0 & R.FoSEx==1)=1; %>2 or >0
                    errorTrial(find(errorTrial)+1)=1; % exclude for exe2
                    errorTrial(R.numError>0 & R.FoSEx==2)=1;% trials where only mistake on exe2
                    % Loop through conditions.
                    idxCond = 1; c_idx = 1; % idxCond for all, c_idx for those which end up being regressors
                    for exe = 1:2 % execution 1 and 2
                        if exe==1
                            loopSeq = 12;
                        else
                            loopSeq = 13; % 7 added for all errors
                        end
                        for c = 1:loopSeq
                            calcRegress = 0;
                            if idxCond<=numel(num_seq)*2 % if done all sequences in first and second exe
                                idx = find(R.seqNumb==c & errorTrial==0 & R.FoSEx==exe); 
                            else % error
                                idx = find(errorTrial==1); % find indx of all CORRECT trials in run - 1:6 trained; 7-12 untrained
                            end
                            if ~isempty(idx) && c<=numel(num_seq)
                                condName = sprintf('SeqNumb-%d-Ex%d',R.seqNumb(idx(1)),exe);
                                S.isError = 0;
                                calcRegress=1;
                            elseif ~isempty(idx) && c==numel(num_seq)+1 % only on second exe
                                condName = 'Error';
                                S.isError = 1;
                                calcRegress=1;
                            end        
    ...
    case 'GLM_estimate_RepSup_error'
        % Estimate the GLM from the appropriate SPM.mat file. 
        % Make GLM files with case 'GLM_make'.
        vararginoptions(varargin,{'sn','sessN'});
        for ss = sessN
            for s = sn
                % Load files
                load(fullfile(glmFoSExErrorDir{ss},subj_name{s},'SPM.mat')); % add glm type
                SPM.swd = fullfile(glmFoSExErrorDir{ss},subj_name{s});
                % Run the GLM.
                spm_rwls_spm(SPM);
            end; % sn
        end; %sessN
        % for checking -returns img of head movements and corrected sd vals
        % spm_rwls_resstats(SPM)
    case 'GLM_contrast_RepSup_error'
         % enter sn, sessN #
        % 1:   Trained seq vs. rest
        % 2:   Novel seq vs. rest   
        sn=[5:9,11:31];
        ...
        for ss=sessN
            % Loop through subjects.
            for s = sn  
                glmSubjDir = fullfile(glmFoSExErrorDir{ss},subj_name{s}); % add glm type
                cd(glmSubjDir);
                load SPM;
                SPM = rmfield(SPM,'xCon');
                T   = load('SPM_info.mat');
                if ~strcmp(SPM.swd,fullfile(glmFoSExErrorDir{ss},subj_name{s}));
                    SPM=sml1_imana_prep('HOUSEKEEPING_renameSPM','sn',s,'SPM',SPM,'glmDir',glmFoSExErrorDir{ss});
                end
                %_____t contrast for trained seq 1st vs. rest
                con                                             = zeros(1,size(SPM.xX.X,2));
                con(:,T.seqNumb<7 & T.FoSEx==1 & T.isError==0)  = 1;
                con                                             = con/sum(con);
                SPM.xCon(1)                                     = spm_FcUtil('Set',sprintf('TrainSeq_1st'), 'T', 'c',con',SPM.xX.xKXs);
                
                %_____t contrast for trained seq 2nd vs. rest
                con                                             = zeros(1,size(SPM.xX.X,2));
                con(:,T.seqNumb<7 & T.FoSEx==2 & T.isError==0)  = 1;
                con                                             = con/sum(con);
                SPM.xCon(2)                                     = spm_FcUtil('Set',sprintf('TrainSeq_2nd'), 'T', 'c',con',SPM.xX.xKXs);
                
                %_____t contrast for untrained seq 1st vs. rest
                con                                             = zeros(1,size(SPM.xX.X,2));
                con(:,T.seqNumb>6 & T.FoSEx==1 & T.isError==0)  = 1;
                con                                             = con/sum(con);
                SPM.xCon(3)                                     = spm_FcUtil('Set',sprintf('UntrainSeq_1st'), 'T', 'c',con',SPM.xX.xKXs);
                
                %_____t contrast for untrained seq 2nd vs. rest
                con                                             = zeros(1,size(SPM.xX.X,2));
                con(:,T.seqNumb>6 & T.FoSEx==2 & T.isError==0)  = 1;
                con                                             = con/sum(con);
                SPM.xCon(4)                                     = spm_FcUtil('Set',sprintf('UntrainSeq_2nd'), 'T', 'c',con',SPM.xX.xKXs);
    ...
    case 'BEH_check_errors_BN'
       ...
       for ss=1:4
           for s=[5:9,11:31];
               ...
               L = getrow(D,D.ScanSess==ss);    % only blocks of that scan session
               if ss == 4
                   R = getrow(L,L.blockType==9); %blockType==9 - func imaging run without metronome
               else
                   R = getrow(L,L.blockType==3); %blockType==3 - funct imaging run with metornome
               end
               BN = unique(R.BN);
               for i=1:8
                   F = getrow(R,R.BN == BN(i));
                   T.ER = [sum(F.isError(F.seqType==1))/sum(F.seqType==1); sum(F.isError(F.seqType==2))/sum(F.seqType==2)];
                   T.seqType = [1;2];
                   T.sn = [s;s];
                   T.sessN = [ss;ss];
                   T.BN = [i;i];
               ...
               [~,i1]=max(TT.ER(TT.seqType==1));
               [~,i2]=max(TT.ER(TT.seqType==2));
               [~,i3]=max(TT.ER); % overall
               K.BN_maxErr = [i1;i2;i3];
               K.SN = [s;s;s];
               K.sessN = [ss;ss;ss];
               K.seqType = [1;2;3];
               KK = addstruct(KK,K);
               TTT = addstruct(TTT,TT);
               fprintf('Sess-%d sn-%d\n',ss,s);
           end
       end
       save(fullfile(baseDir,'behavioral_data','analyze','scanner_errors_block'),'-struct','KK');
       save(fullfile(baseDir,'behavioral_data','analyze','scanner_errors'),'-struct','TTT');
       TTT = normData(TTT,'ER');
       sty = style.custom({'red','blue'},'markersize',8);
       figure
       plt.line(TTT.BN,TTT.normER,'split',TTT.seqType,'style',sty);
    ...
    case 'GLM_make_sess_error'
        ...
        for ss=sessN % session
            ...
            for s = sn
                ...
                % Do some subject structure fields.
                dircheck(fullfile(glmSessErrorDir{ss},subj_name{s})); % add glm type
                J.dir 			 = {fullfile(glmSessErrorDir{ss},subj_name{s})};
                J.timing.units   = 'secs';                                      % timing unit that all timing in model will be
                J.timing.RT 	 = 1.0;                                         % TR (in seconds, as per 'J.timing.units')
                J.timing.fmri_t  = 16;
                J.timing.fmri_t0 = 1;
                ...
                for r = 1:numruns_task_sess
                    ...
                    for c = 1:numel(num_seq)+1
                        if c<=numel(num_seq)
                            ...
                            idx = find(R.isError==1); % find indx of all CORRECT trials in run - 1:6 trained; 7-12 untrained
                            condName = 'Error';
                            S.isError = 1;
                        end
                        J.sess(r).cond(c).name 	   = condName;
                ...
                for r = 1:numruns_task_sess
                    ...
                    for c = 1:numel(num_seq)+1
                        ...
                        if c<=numel(num_seq)
                            ...
                            idx = find(R.seqNumb==c & R.numError<1);
                            %idx = find(R.seqNumb==c & R.isError==0); % find indx of all CORRECT trials in run - 1:6 trained; 7-12 untrained
                        else % error
                            %idx = find(R.numError>=numError+2);
                            idx = find(R.numError>=1);
        ...
        for ss=sessN
            ...
            for s = sn  
             ...
             %   end
                ...
                % (4) t contrast of all seq vs. rest
                con                = zeros(1,size(SPM.xX.X,2));
                con(:,T.seqNumb>0 & T.seqNumb<13 & T.isError==0)  = 1;
                con                = con/sum(con);
                SPM.xCon(end+1)    = spm_FcUtil('Set',sprintf('AllSeq'), 'T', 'c',con',SPM.xX.xKXs);
    ...
    case '3h_GLM_make_perm_error'
           % here permute the order
    case 'GLM_perm_all'
        glm=3;
        sn = [5:9,11:31];
        sessN = 1:2;
        nPerm = 1:20;
        vararginoptions(varargin,{'sn','glm','sessN','nPerm'});
        for n=nPerm
            for ss=sessN
                clc;
                for s=sn
                    tElapsed = tic;
                    sml1_imana_prep('GLM_make_sess_perm','sn',s,'glm',glm,'sessN',ss,'numPerm',n);
                    sml1_imana_prep('GLM_estimate_sess_perm','sn',s,'sessN',ss,'numPerm',n);
                    fprintf('********* Done permutation %d all %s sess-%d.*********\n\n\n',n,subj_name{s},ss);
                    toc(tElapsed);
                end
            end
        end
    case 'GLM_make_sess_perm'
        % makes the GLM file for each subject, and a corresponding 
        % SPM_info.mat file. The latter file contains nice summary
        % information of the model regressors, condition names, etc.
        % use permuted error / correct trials
        sessN = 1:4;
        sn = [5:9,11:31];
        glm = 3;    %1/2/3
        numPerm = 1;
        vararginoptions(varargin,{'sn','glm','sessN','numPerm'});
        % Set some constants.
        prefix       = 'u';
        T            = [];
        dur          = 2.5;                                                 % secs (length of task dur, not trial dur)
        % adjusting hrf per subject & session based on extracted timeseries!
        delay     = [0.5 1 1 0.5 1 0 0 0.5 1 1 0.5 1 1 1 1 0 1 1.5 1 1 1 1 1 1 0.5 0 1 1 1 1 1];
        colName = sprintf('isError_perm%d',numPerm);
        announceTime = 0;                                                 % length of task announce time - currently not used
        % Gather appropriate GLM presets.
        switch glm
            case 1  % wls
        ...
        for ss=sessN % session
            ...
            for s = sn
                tElapsed = tic;
                D = dload(fullfile(baseDir,'test_behaviour',['sml1_',subj_name{s},'.dat']));
                T=[];         
                % Do some subject structure fields.
                dircheck(fullfile(glmPermDir{numPerm}{ss},subj_name{s})); % add glm type
                J.dir            = {fullfile(glmPermDir{numPerm}{ss},subj_name{s})};
                J.timing.units   = 'secs';                                      % timing unit that all timing in model will be
                J.timing.RT      = 1.0;                                         % TR (in seconds, as per 'J.timing.units')
                J.timing.fmri_t  = 16;
                J.timing.fmri_t0 = 1;               
                ...
                for r = 1:numruns_task_sess
                    ...
                    for c = 1:numel(num_seq)+1
                        calcRegress=0;
                        if c<=numel(num_seq)
                            idx = find(R.seqNumb==c & R.(colName)==0);
                            %idx = find(R.seqNumb==c & R.isError==0); % find indx of all CORRECT trials in run - 1:6 trained; 7-12 untrained
                        else % error
                            idx = find(R.(colName)==1);
                            %idx = find(R.isError==1); % find indx of all INCORRECT trials in run - 1:6 trained; 7-12 untrained
                        end
                        if ~isempty(idx) && c<=numel(num_seq)
                            condName = sprintf('SeqNumb-%d',R.seqNumb(idx(1)));
    ...
    case 'GLM_estimate_sess_perm'
        % Estimate the GLM from the appropriate SPM.mat file. 
        % Make GLM files with case 'GLM_make'.
        numPerm = 1;
        vararginoptions(varargin,{'sn','sessN','numPerm'});
        for ss = sessN
            for s = sn
                % Load files
                load(fullfile(glmPermDir{numPerm}{ss},subj_name{s},'SPM.mat')); % add glm type
                SPM.swd = fullfile(glmPermDir{numPerm}{ss},subj_name{s});
                % Run the GLM.
                spm_rwls_spm(SPM);
            end; % sn
        end; %sessN
    ...
    case 'ROI_define'                                                       % STEP 5.1   :  Define ROIs
        ...
        vararginoptions(varargin,{'sn','parcelType','hemi'});
        for s=sn
            R=[];
            idx=0;
            for h=hemi
                ...
                switch (parcelType)
                    ...
                    case 'Yokoi_clusters'
                        C=caret_load(fullfile(caretDir,'fsaverage_sym',hemName{h},sprintf('%s.Yokoi2019.10cluster.paint',hem{h}))); % freesurfer
                        tessels=unique(C.data);
                        tessels = tessels(tessels~=0);
                        %if h==2,
                        %    idx=C.num_paintnames; % append to the first hemisphere indices
                       % end
                        for i=1:length(tessels)
                            R{i+idx}.type='surf_nodes';
                            R{i+idx}.location=find(C.data(:,1)==tessels(i));
                            R{i+idx}.white=fullfile(caretSubjDir,hemName{h},[hem{h} '.WHITE.coord']);
                            R{i+idx}.pial=fullfile(caretSubjDir,hemName{h},[hem{h} '.PIAL.coord']);
                            R{i+idx}.topo=fullfile(caretSubjDir,hemName{h},[hem{h} '.CLOSED.topo']);
                          %  R{i+idx}.flat=fullfile(caretDir,'fsaverage_sym',hemName{h},[hem{h} '.FLAT.coord']);
                            ...
                            R{i+idx}.image=file;
                            R{i+idx}.file=file;
                            R{i+idx}.name=sprintf('Yokoi_%s-%s',C.paintnames{tessels(i)},hem{h});
                        end;
                        idx = numel(tessels);
                end
            end;
    ...
    case 'ROI_define_wb'
        ...
        vararginoptions(varargin,{'sn','nNodes'});
        parcelType = sprintf('tesselsWB_%d',nNodes);
        for s=sn
            R=[];
            idx=1;
            ...
            for h=1:2
                G = gifti(fullfile(surfDir,sprintf('Icosahedron-%d.164k.%s.label.gii',nNodes,hemI{h})));
                nReg = numel(unique(G.cdata))-1; % exclude 0 - medial wall
                for r=1:nReg
                    R{idx}.type     = 'surf_nodes'; 
                    R{idx}.location = find(G.cdata(:,1)==r);
                    R{idx}.white    = fullfile(subjDir,sprintf('%s.%s.white.164k.surf.gii',subj_name{s},hemI{h}));
                    R{idx}.pial     = fullfile(subjDir,sprintf('%s.%s.pial.164k.surf.gii',subj_name{s},hemI{h}));
                    R{idx}.linedef  = [5,0,1];
                    R{idx}.image    = mask;
                    R{idx}.name     = sprintf('%s.tessel-%d',hemI{h},r);
                    idx = idx+1;
                end
            end
            R = region_calcregions(R);
    ...
    case 'ROI_Yokoi_surface'
      % project Yokoi surface to workbench
      hemi = 2;
      vararginoptions(varargin,{'hemi'}); % which hemisphere to work on
      
      switch hemi
          case 1 % for left hemisphere directly from caret
              INfile = fullfile(caretDir,'fsaverage_sym','LeftHem','lh.Yokoi2019.10cluster.paint');
              OUTfile = fullfile(wbDir,'FS_LR_164','L.Yokoi2019.10cluster.label.gii');
              atlasDir = '/Users/eberlot/Documents/Data/Atlas_templates/standard_mesh';
              surf_resamplePaint(1,INfile,OUTfile,atlasDir,'resolution','164k');
          case 2
              cd(regDir);
              INfile = dir('Yokoi_*-rh.nii');
              subjDir = fullfile(wbDir,subj_name{5}); % created from subject 5
              white   = fullfile(subjDir,sprintf('%s.%s.white.164k.surf.gii',subj_name{5},hemI{2}));
              pial    = fullfile(subjDir,sprintf('%s.%s.pial.164k.surf.gii',subj_name{5},hemI{2}));
              C1      = gifti(white);
              C2      = gifti(pial);
              for f=1:size(INfile)
                  images{f} = fullfile(regDir,INfile(f).name);
              end;
              G = surf_vol2surf(C1.vertices,C2.vertices,images,'anatomicalStruct','CortexRight','depth',.05);
              save(G,fullfile(wbDir,'FS_LR_164','R.Yokoi2019.10cluster.func.gii'));
              surfFile = fullfile(wbDir,'FS_LR_164','fs_LR.164k.R.inflated.surf.gii');
              surf_smooth(fullfile(wbDir,'FS_LR_164','R.Yokoi2019.10cluster.func.gii'),'surf',surfFile,'kernel',1);
              % make into a label file
              G1 = gifti(fullfile(wbDir,'FS_LR_164','sR.Yokoi2019.10cluster.func.gii'));
              newData = nansum(G1.cdata,2);
              newData(newData>0)=1;
              [~,i2]=max(G1.cdata');
              data = newData.*i2';
              G2 = gifti(fullfile(wbDir,'FS_LR_164','L.Yokoi2019.10cluster.label.gii'));
              labelRGBA = G2.labels.rgba([1,2,3,4,7,9,10,11],:);
              labelNames = G2.labels.name([1,2,3,4,7,9,10,11]);
              G = surf_makeLabelGifti(data,'anatomicalStruct','CortexRight','labelNames',labelNames,'labelRGBA',labelRGBA);
              save(G,fullfile(wbDir,'FS_LR_164','R.Yokoi2019.10cluster.label.gii'));
      end
    ...
    case 'subjStruct'
        ...
        keyboard;
    case 'HOUSEKEEPING_renameSPM' 	% rename SPM directories - to do - new FUNC
        vararginoptions(varargin,{'sn','SPM','glmDir'});
        newGLMDir   = fullfile(glmDir,subj_name{sn});
        newRawDir   = fullfile(imagingDir,subj_name{sn});
        SPM=spmj_move_rawdata(SPM,newRawDir);
        SPM.swd=fullfile(newGLMDir);
        save(fullfile(newGLMDir,'SPM.mat'),'SPM','-v7.3');
        varargout{1} = SPM;
    case 'run_job'
        %sml1_imana_prep('GLM_sess_error_all','sn',[20,29],'sessN',1);
        %sml1_imana_prep('GLM_sess_error_all','sn',29,'sessN',2);
        sml1_imana_prep('GLM_sess_error_all','sn',28,'sessN',1:3);
        sml1_imana_prep('GLM_sess_error_all','sn',[5:9,11:31],'sessN',4);
        sml1_imana_dist('PSC_create','errorNum',6,'sessN',4);
        sml1_imana_dist('PSC_create','errorNum',6,'sessN',1:2,'sn',29);
        sml1_imana_dist('PSC_create','errorNum',6,'sessN',1,'sn',20);
        sml1_imana_dist('BETA_get','sn',[5:9,11:27,29:31],'sessN',4,'numError',6);
        sml1_imana_dist('BETA_get','sn',29,'sessN',1:2,'numError',6);
        sml1_imana_dist('BETA_get','sn',20,'sessN',1,'numError',6);
        %sml1_imana_dist('BETA_combineGroup','numError',6);
    case 'run_job1'
        sml1_imana_prep('GLM_sess_error_all','sn',[22:27,30:31],'sessN',1);
        sml1_imana_prep('GLM_sess_error_all','sn',[5:9,11:27,30:31],'sessN',2);
        sml1_imana_prep('GLM_sess_error_all','sn',[20:27,29:31],'sessN',3);
        %sml1_imana_prep('GLM_make_sess_error2','sn',[20:27,29:31],'glm',3,'sessN',3,'numError',6);
        %sml1_imana_prep('GLM_sess_error_all','sn',29,'glm',3,'sessN',1);
        %sml1_imana_dist('BETA_get','sn',29,'sessN',1,'numError',6);
        %sml1_imana_dist('BETA_combineGroup','numError',6,'sessN',1);
        %sml1_imana_dist('BETA_stats','roi',[1:3,7,8],'numError',6);
        sml_plasticity('PCM:corrModels_nonlinear_bCovCompare','numError',6,'withinCov','individual');
    otherwise
