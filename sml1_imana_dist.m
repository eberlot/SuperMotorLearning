...
switch(what)
    ...
    case 'PCM_constructModelFamily_seqType' % THIS ONE
        runEffect  = 'fixed';
        beta_choice = 'mw';
        algorithm='NR'; % minimize or NR
        parcelType='162tessels'; % Brodmann or 162tessels
        ...
        save(fullfile(pcmDir,sprintf('ModelFamily_Fit_%s_fing%s.mat',parcelType,fingType)),'-struct','AllReg');
        save(fullfile(pcmDir,sprintf('ModelFamily_Stats_%s_fing%s.mat',parcelType,fingType)),'-struct','KK');
...
% seqType is index whether trained (1) or untrained (2)
    ...
    M{4}(:,1:6)    = A;       % Trained sequence patterns
end
