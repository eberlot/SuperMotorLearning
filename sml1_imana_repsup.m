...
switch(what)
    ...
    case 'PSC_group_cSPM_RepSup'
        ...
        sqrtTransform=[0,0,0,0]; % Should you take ssqrt before submitting? 
                                % no for psc
        SummaryName = sprintf('.summary_RS_psc_sess%d.metric',sessN);
        hemi = [1 2];
        ...
        for h=hemi
            ...
            for i=1:length(SPMname);
                filenames{i}=[surfaceGroupDir filesep hem{h} '.RS_' SPMname{i} '_sess' num2str(sessN) '.metric']; % no smoothing
    ...
    case 'PSC_group_smooth_RepSup'
        ...
        for h=1:2
            ...
            for i=1:length(sessN);
                filename=[surfaceGroupDir filesep hem{h} '.summary_RS_psc_sess' num2str(sessN) '.metric']; % unsmoothed
    ...
    case 'SURF_flatmap_RS_absolute'
        ...
        for h=1:2
            ...
            name={sprintf('RS_absolute_psc_sess%d',sessN)};
            ...
            if smooth == 1
                caret_save([surfaceGroupDir filesep 's' hem{h} sprintf('.RS_sess%d.RGB_paint',sessN)],C);
            else
                caret_save([surfaceGroupDir filesep hem{h} sprintf('.RS_sess%d.RGB_paint',sessN)],C);
            end
        end;
       ...
       % make a summary stats structure from betas for each subject / ROI
        ...
        for ss=sessN
            if strcmp(parcelType,'cortex_buckner')
                T = load(fullfile(regDir,sprintf('betas_FoSEx_%s_sess%d.mat',parcelType,ss))); % loads region data (T)    
            else
            ...
            % % save
            if strcmp(parcelType,'cortex_buckner')
                save(fullfile(regDir,sprintf('stats_FoSEx_%s_%sPW_sess%d.mat',parcelType,betaChoice,ss)),'-struct','To');
            else
                save(fullfile(regDir,sprintf('stats_FoSEx_%sPW_sess%d.mat',betaChoice,ss)),'-struct','To');
            end  
            fprintf('\nDone stats for session %d.\n',ss);
        end
    case 'ROI_stats_redIPM_exe' % TO DO
        ...
        roi = [1:16];
        betaChoice = 'multi'; % uni, multi or raw
        regType = 'cortex'; % cortex or striatum
        vararginoptions(varargin,{'sn','sessN','roi','betaChoice','regType'});
        ...
        for ss=sessN
            switch regType
                case 'cortex'
                    T = load(fullfile(regDir,sprintf('betas_FoSEx_sess%d.mat',ss))); % loads region data (T)   
                case 'striatum'
                    T = load(fullfile(regDir,sprintf('betas_BG-striatum_FoSEx_sess%d.mat',ss))); % loads region data (T)   
            end
            % output structures
            Ts = [];
            To = []; 
            ...
            switch regType
                case 'cortex'
                    save(fullfile(regDir,sprintf('reduced_IPM_FoSEx_sess%d.mat',ss)),'-struct','To');
                case 'striatum'
                    save(fullfile(BGDir,'stats',sprintf('reduced_IPM_FoSEx_sess%d.mat',ss)),'-struct','To');
            end
            fprintf('\nDone for session %d.\n',ss);
        end
    ...
    case 'CALC_act_dist' %-------------------------- ACTIVATION, MAHALANOBIS DIST  ----------------------
        ...
        for ss = sessN % do per session number
              if strcmp(parcelType,'cortex_buckner')
                    D = load(fullfile(regDir,sprintf('stats_FoSEx_%s_%s_sess%d.mat',parcelType,betaChoice,ss))); % loads region data (D)
                    T = load(fullfile(regDir,sprintf('betas_FoSEx_%s_sess%d.mat',parcelType,ss))); % loads region data (T)
              else
       ...
       % % save % change name
         if strcmp(parcelType,'cortex_buckner')
            ...
            save(fullfile(repSupDir,sprintf('RepSup_stats_%s.mat',betaChoice)),'-struct','Stats');
         end
    case 'PLOT_beta'
    ...
    case 'PLOT_psc'
       ...
       for st=1:2 % sequence type
           ...
           for f = roi
               ...
               plt.line([S.speed S.sessN],var,'split',S.FoSEx,'subset',S.roi==f,'style',style,'leg',{'1st','2nd'});
               drawline(0,'dir','horz');
               plt.match('y');
               if f==1
    ...
    case 'PLOT_dist'
       ...
       for f = 1:numel(roi)
           subplot(1,numel(roi),f);
           plt.line([S.speed S.sessN],S.dist_train,'split',S.FoSEx,'subset',S.roi==f,'style',styTrained_exe,'leg',{'1st','2nd'});
           plt.match('y');
           if f==1
       ...
       for f = 1:numel(roi)
           subplot(1,numel(roi),f);
           plt.line([S.speed S.sessN],S.dist_untrain,'split',S.FoSEx,'subset',S.roi==f,'style',styUntrained_exe,'leg',{'1st','2nd'});
           plt.match('y');
           if f==1
       ...
       for f = 1:numel(roi)
           subplot(1,numel(roi),f);
           plt.line([S.speed S.sessN],S.dist_cross,'split',S.FoSEx,'subset',S.roi==f,'style',stySeqType_exe,'leg',{'1st','2nd'});
           plt.match('y');
           if f==1
    ...
    case 'CALC_corrDist' %--------------------------- CORRELATION DISTANCE -------------------------------
        ...
        for  ss = sessN
            if strcmp(parcelType,'cortex_buckner')
                D   = load(fullfile(regDir,sprintf('betas_FoSEx_%s_sess%d.mat',parcelType,ss)));
            else
                D   = load(fullfile(regDir,sprintf('betas_FoSEx_sess%d.mat',ss)));
            end
            ...
            for roi = reg;
                for s = sn;
                    ...
                    fprintf('Done ses%d %s %s\n',ss,subj_name{s},regname{roi});
                end;
            end;
        end;
        if strcmp(parcelType,'cortex_buckner')
            ...
            save(fullfile(repSupDir,sprintf('corrDist_RS_meanSubtract%d.mat',subtract_mean)),'-struct','SAll');
            save(fullfile(repSupDir,sprintf('corrDist_seqType_RS_meanSubtract%d.mat',subtract_mean)),'-struct','STAll');
        end
    case 'PLOT_corrDist'
        roi=[1:8];
        subtract_mean=0;
        ...
        vararginoptions(varargin,{'roi','subtract_mean','distType','parcelType'});
    ...
    case 'CALC_subtract_actDist' %--------------------------- REPSUP AS SUBTRACTION: 1st - 2nd ---------------------
       ...
       % % save
        save(fullfile(repSupDir,sprintf('RepSup_subtract_actDist_%s.mat',betaChoice)),'-struct','P');    
    case 'PLOT_subtract_beta'
       ...
       for f = 1:numel(roi)
           subplot(1,numel(roi),f);
           plt.line([S.speed S.sessN],S.subtr_beta,'split',S.seq,'subset',S.roi==f,'style',stySeq,'leg',{'Trained','Untrained'});
           plt.match('y');
           drawline(0,'dir','horz');
    ...
    case 'PLOT_subtract_psc'
       ...
       for f = 1:numel(roi)
           subplot(1,numel(roi),f);
           plt.line([S.speed S.sessN],S.subtr_psc,'split',S.seq,'subset',S.roi==f,'style',stySeq,'leg',{'Trained','Untrained'});
           drawline(0,'dir','horz');
           plt.match('y');
          % ylim([-0.1 1.1]); hold on; drawline(1,'dir','horz'); drawline(0,'dir','horz');
           if f==1
    ...
    case 'PLOT_subtract_dist'
       ...
       for f = 1:numel(roi)
           subplot(1,numel(roi),f);
           plt.line([S.speed S.sessN],S.subtr_dist,'split',S.seq,'subset',S.roi==f,'style',stySeq,'leg',{'Trained','Untrained'});
          % ylim([-0.1 1.1]); hold on; drawline(1,'dir','horz'); drawline(0,'dir','horz');
           if f==1
    ...
    case 'CALC_ratio_actDist' %--------------------------- REPSUP AS RATIO: 2nd/1st --------------------------
       ...
       % % save
        save(fullfile(repSupDir,sprintf('RepSup_percent_actDist_%s.mat',betaChoice)),'-struct','P');
    case 'PLOT_ratio_beta'
        betaChoice = 'multiPW';
        roi=[1:8];
        vararginoptions(varargin,{'betaChoice','roi'});
        
        S = load(fullfile(repSupDir,sprintf('RepSup_percent_actDist_%s.mat',betaChoice)));

       figure
       for f = 1:numel(roi)
           subplot(1,numel(roi),f);
           plt.line([S.speed S.sessN],S.psc_beta,'split',S.seq,'subset',S.roi==f,'style',stySeq,'leg',{'Trained','Untrained'});
           ylim([-0.1 1.1]); hold on; drawline(1,'dir','horz'); drawline(0,'dir','horz');
           if f==1
    ...
    case 'PLOT_ratio_psc'
       ...
       for f = roi
           subplot(1,numel(roi),indx);
           plt.line([S.speed S.sessN],S.psc_psc,'split',S.seq,'subset',S.roi==f,'style',stySeq,'leg',{'Trained','Untrained'});
           ylim([-0.1 1.1]); hold on; drawline(1,'dir','horz'); drawline(0,'dir','horz');
           if indx==1
    ...
    case 'PLOT_ratio_dist'
       ...
       for f = 1:numel(roi)
           subplot(1,numel(roi),f);
           plt.line([S.speed S.sessN],S.psc_dist,'split',S.seq,'subset',S.roi==f,'style',stySeq,'leg',{'Trained','Untrained'});
           hold on; drawline(1,'dir','horz'); drawline(0,'dir','horz');
           if f==1
        ...
        % MDS from activation for each sequence type from rest 
         ...
         for ss=sessN
             switch regType
                 ...
                 case 'striatum'
                     T=load(fullfile(BGDir,'stats',sprintf('stats_FoSEx_BG-striatum_%sPW_sess%d.mat',betaChoice,ss)));
        ...
        for ss=sessN
            switch regType
                 case 'cortex'
                     ...
                     regLabel = regname_cortex;
                 case 'striatum'
                     T=load(fullfile(BGDir,'stats',sprintf('reduced_IPM_FoSEx_sess%d.mat',ss)));
                     regLabel = {'Caudate','Putamen'};
             end
            T=load(fullfile(regDir,sprintf('reduced_IPM_FoSEx_sess%d.mat',ss)));
            for r=roi
    ...
    case 'job1'
        sml1_imana_repsup('SEARCH_run_LDC','sn',25,'sessN',1,'repInd',2);
        sml1_imana_repsup('SEARCH_run_LDC','sn',[6:9,11:25],'sessN',3,'repInd',1);
    case 'job2'
    ...
    case 'SEARCH_check'
        ...
        for ss=sessN
            for s=sn
                for rep=1:2
                dir = fullfile(baseDir,'glmFoSEx_old',sprintf('glmFoSEx%d',ss),subj_name{s},sprintf('%s_sess%d_exe%d_LDC.nii',subj_name{s},ss,rep));
                if exist(dir)
